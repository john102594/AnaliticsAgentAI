...
class DownloadDesperdicios:
    def __init__(self, config_path="config.json"):
        self.config_path = config_path
        self.config = self._load_config()
        self.base_dir = self.config.get("descargas_dir", "descargas")
        self.descargas_dir = os.path.join(self.base_dir, "desperdicios")
    ...
    def descargar(self, fecha: str, turno: int):
        ...
        # Buscar config de desperdicios
        reportes_config = self.config.get("server", {}).get("reports", [])
        cfg = next((r for r in reportes_config if r.get("id") == "desperdicios"), None)
        
        if not cfg:
            print("Configuración de 'desperdicios' no encontrada en config.json.")
            return None

        url_pattern = cfg.get("url_pattern")
        filename_pattern = cfg.get("filename_pattern")
        
        url = url_pattern.format(fecha=fecha_url, turno=turno)
        nombre_archivo = filename_pattern.format(fecha=fecha_url, turno=turno)
        ...
        print(f"--- Iniciando descarga de Desperdicios ---")
        
        if os.path.exists(ruta_archivo):
            print(f"El archivo {nombre_archivo} ya existe en desperdicios: {ruta_archivo}")
            return ruta_archivo
        ...
        try:
            ...
            if respuesta.status_code == 200:
                ...
                print(f"Error al descargar Desperdicios. Código: {respuesta.status_code}")
                return None

        except Exception as e:
            print(f"Error procesando la solicitud para Desperdicios: {str(e)}")
            return None
        finally:
            if sesion:
                sesion.close()
...
def main():
    parser = argparse.ArgumentParser(description="Descarga reporte de Desperdicios por Turno.")
    ...
    downloader = DownloadDesperdicios()
