...
class DownloadTPR:
    def __init__(self, config_path="config.json"):
        ...
        self.descargas_dir = os.path.join(self.config.get("descargas_dir", "descargas"), "TPR")
    ...
    def descargar(self, fecha: str):
        ...
        # Buscar config de TPR
        reportes_config = self.config.get("server", {}).get("reports", [])
        cfg = next((r for r in reportes_config if r.get("id") == "tpr"), None)
        
        if not cfg:
            print("Configuración de 'tpr' no encontrada en config.json.")
            return []
        ...
        print(f"--- Iniciando descarga de Informe TP Restriccion ---")
        ...
        try:
            ...
            print(f"Error procesando la solicitud para TPR: {str(e)}")
        finally:
            if sesion:
                sesion.close()
